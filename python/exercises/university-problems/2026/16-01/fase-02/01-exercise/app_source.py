...
class TallerBicicletas:
    ...
    def registrar_salida_taller(self, serial, hora):
        """Registra la salida."""
        if buscar_serial(serial):
            bike_found = self._bicicletas[serial]
            if bike_found.registrar_salida(hora):
                self._bicicletas[serial] = bike_found
                return True
        return False
    ...
    def mostrar_bicicletas(self):
        ...
        if len(self._bicicletas) > 0:
            for v in self._bicicletas.values():
                v.mostrar_info()
                print("\n")
...
class Bicicleta:
    ...
    def registrar_salida(self, hora_salida):
        ...
        if isinstance(hora_salida, Hora):
...
def reg_salida_bicicleta(serial_input, hora_salida_input):
    if validar_input(serial_input) and validar_input(hora_salida_input):
        nueva_hora = conv_texto_hora(hora_salida_input)
        if not nueva_hora is None and len(serial_input) <= 15:
            reg_ok = TALLER_BICICLETAS.registrar_salida_taller(
                serial_input, nueva_hora)
            return reg_ok
    return False
...
def buscar_serial(serial_input: str):
    serial_input = serial_input.upper()
    if validar_input(serial_input):
        return TALLER_BICICLETAS.buscar_serial(serial_input)
    return False
